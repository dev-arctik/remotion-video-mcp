      ...
      try {
        ...
        const result = {
          ...
          next_steps: [
            ...
            `  const { isOnBeat, isDownbeat, beatIndex } = useBeat({ tier: 'downbeat', tolerance: 1 });`,
            ``,
            `⛔ MOTION REST STATE — important:`,
            `  Use beat data for SCENE CUTS (durationFrames lands on a downbeat) and for`,
            `  ENTRANCE TIMING (animation.delay = beat frame - sceneStart). Then text HOLDS STILL.`,
            `  ❌ DO NOT apply useBeat().pulse to text scale/transform — pulses every downbeat = throbbing on titles.`,
            `  ✅ The .pulse value is for DECORATIVE shapes only (rings, particles), never on text.`,
            `  NEVER use useBeat({ tier: 'beat' }) — that's per-quarter-note throbbing at any BPM.`,
            ``,
